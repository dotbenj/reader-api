...
export class RecommandationsService {
  ...
  //   $(`
  //     body > 
  //     div#myanimelist:nth-child(1) > 
  //     div.wrapper:nth-child(3) > 
  //     div#contentWrapper:nth-child(3) > 
  //     div#content:nth-child(2) > 
  //     table:nth-child(2) > 
  //     tbody > tr > td:nth-child(1) > 
  //     table:nth-child(2) > tbody > 
  //     tr:nth-child(1)`).find('td.borderClass > div:nth-child(1) > a').each(async (index: number, elem) => {
  //     const profile = $(elem).attr('href').split('/')[2];
  //     const html2 = await axios.default.get(`https://myanimelist.net/mangalist/${profile}?status=7`);
  ...
  //         console.log('manga url', manga.manga_url);
  //         queries.push({
